...
//class Bomberman{
//public:
//	Bomberman(sf::Vector2i spawnTile, std::string filename, sf::Clock& clock) {
...
//		spriteRect.left = sprite.getPosition().x + 5;
//		spriteRect.top = sprite.getPosition().y + 25;
//		spriteRect.width = sprite.getTextureRect().width - 2.5;
//		spriteRect.height = sprite.getTextureRect().height - 20;
//		return worldToTileCoord(sf::Vector2f(spriteRect.left + spriteRect.width/2, spriteRect.top + spriteRect.height/2));
//	}
...
//class Tile{
//public:
//	enum TileType{Grass, Box, Solid, Portal, Drop};
//	Tile() {
//		holdingBomb = false;
...
//	}
//	bool isWalkable() {
//		return (tileType == Grass || tileType == Portal || tileType == Drop);
//	}
...
//	}
//	bool isDestructible() {
//		return (tileType == Box || tileType == Drop);
//	}
//	bool isHoldingDrop() {
//		return holdingDrop;
//	}
...
//		this->map = map;
//		animator = thor::Animator<sf::Sprite, std::string>();
...
//class TileManager{
//public:
//	enum TileData{Grass, Box, Solid, Portal, HEnemy, VEnemy, HVEnemy, SEnemy};
...
//					break;
//				case Portal:
//					map[i][j].setTileType(Tile::Portal);
//					map[i][j].getSprite().setTextureRect(sf::IntRect(0, 2*TILE_HEIGHT, TILE_WIDTH, TILE_HEIGHT));
...
//					map[i][j].getSecondarySprite().setColor(sf::Color(255,0,128,128));
//					portalIdx = sf::Vector2i(j, i);
//					break;
//				default:
...
//	}
//
//	enum Powers{FireUp, FireDown, BombUp, Skate, Power, Kick, NumberOfPowers};
//	void draw(sf::RenderWindow& window, sf::Time dt) {
//		for (int i=0; i<ROWS; i++) {
//			for (int j=0; j<COLS; j++) {
...
//					}
//				}
//				else if (map[i][j].getTileType() == Tile::Portal) {
//					window.draw(map[i][j].getSprite());
//					window.draw(map[i][j].getSecondarySprite());
//				}
//				else {
...
//
//	void activatePortal() {
//		map[portalIdx.y][portalIdx.x].getSecondarySprite().setColor(sf::Color(0,0,0,0));
//		portalActive = true;
//	}
//
//	bool portalIsActive() {
...
//}
//
//bool checkCollisionForBomberman(Bomberman &b, BombManager &bm, TileManager &tm, bool modifyBM, sf::Time elapsedTime) {
//	if (b.isDead())
//		b.respawn();
//	else {
//		sf::FloatRect spriteRect;
...
//	}
//
//	if(checkCollisionForBomberman(bomberman, bm, tm, true, clock.getElapsedTime())) {
//		bomberman.setPosition(bomberman.getPrevPosition().x, bomberman.getPrevPosition().y);
//	}
//}
//
//void handleInput(Bomberman& player1, Bomberman& player2, BombManager& bm, BombManager& bm2, TileManager &tm, sf::Clock clock) {
//	if (!player1.isDead()) {
//		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
...
//	}
//
//	if(checkCollisionForBomberman(player1, bm, tm, true, clock.getElapsedTime())) {
//		player1.setPosition(player1.getPrevPosition().x, player1.getPrevPosition().y);
//	}
//	else if(checkCollisionForBomberman(player1, bm2, tm, false, clock.getElapsedTime())) {
//		player1.setPosition(player1.getPrevPosition().x, player1.getPrevPosition().y);
//	}
//
//	if (!player2.isDead()) {
...
//	}
//
//	if(checkCollisionForBomberman(player2, bm, tm, false, clock.getElapsedTime())) {
//		player2.setPosition(player2.getPrevPosition().x, player2.getPrevPosition().y);
//	}
//	else if(checkCollisionForBomberman(player2, bm2, tm, true, clock.getElapsedTime())) {
//		player2.setPosition(player2.getPrevPosition().x, player2.getPrevPosition().y);
//	}
//}
//
//void SplashScreen(std::string filename, sf::RenderWindow &window) {
//	sf::Texture texture;
//	texture.loadFromFile(filename);
//	sf::Sprite sprite;
//	sprite.setTexture(texture);
...
//}
//
//void action(Bomberman& player1, Bomberman& player2, BombManager& bm, BombManager& bm2, TileManager &tm, sf::Clock clock) {
//	handleInput(player1, player2, bm, bm2, tm, clock);
//	std::forward_list<Bomb*>* bombs = &bm.getBombs();
...
//}
//
//void action(Bomberman& bomberman, EnemyManager& em, BombManager& bm, TileManager &tm, sf::Clock clock) {
...
//	window.draw(r);
//	sf::Time t = sf::seconds(120);
//	t = t - elapsedTime;
//	std::string mins, secs;
//	mins = "2";
//	if (t.asSeconds() < 120 && t.asSeconds() > 60)
//		mins = "1";
...
//	Bomberman bomberman(sf::Vector2i(1,1), "bomberman.png", clock);
//	Bomberman bomberman2(sf::Vector2i(19,13), "bomberman2.png", clock);
...
//	SplashScreen("splash.png", window);
//	while (window.isOpen())	{
//		sf::Event event;
//		restart = false;
...
//				std::cout << sf::Mouse::getPosition(window).x << " " << sf::Mouse::getPosition(window).y - 40 << std::endl;
...
//					new (&bomberman) Bomberman(sf::Vector2i(1,1), "bomberman.png", clock);
//					new (&bomberman2) Bomberman(sf::Vector2i(19,13), "bomberman2.png", clock);
//					em = EnemyManager("ghosts.png", bomberman, clock);
//					tm.~TileManager();
//					new (&tm) TileManager("level1.txt", powerups, tiles, explosion, em, clock);
//					map = tm.getMap();
//					em.setMap(map);
//					bm.~BombManager();
//					bm2.~BombManager();
...
//		if (restart) continue;
//		if (mode == Arcade)
//			action(bomberman, em, bm, tm, clock);
//		else if (mode == TwoPlayers)
//			action(bomberman, bomberman2, bm, bm2, tm, clock);
//		restart = false;
//		sf::Time dt = animationClock.restart();
//		window.clear(sf::Color(60,100,20));
//		if (mode == Arcade)
//			drawSprites(window, font, bomberman, bm, tm, em, clock.getElapsedTime(), dt);
//		else if (mode == TwoPlayers)
//			drawSprites(window, font, bomberman, bomberman2, bm, bm2, tm, clock.getElapsedTime(), dt);
//		window.display();
//	}
//	return 0;
//}